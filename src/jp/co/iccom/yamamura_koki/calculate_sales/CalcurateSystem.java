...
public class CalcurateSystem {
	...
	public static void main(String[] args) throws IOException {
		...
		for(int i=0; i < filterFile.length; i++){
			...
			try{
				...
				//支店別合計の計算
				if(!fileCalcurate(branchValueResult, rcdFile, 0)){
					return;
				}

				//商品別売上の計算
				if(!fileCalcurate(commodityValueResult, rcdFile, 1)){
					return;
				}
	...
	//売上ファイル読み込み
	private static boolean rcdFileRead(String path, String filterFile, String splitName,
			HashMap<String, String> branchName, HashMap<String, String> commodityName, int branchIndex, int commodityIndex){
		...
		try{
			...
			if(!branchName.containsKey(rcdFile.get(branchIndex))){
				System.out.println(filterFile + "の支店コードが不正です");
				return false;
			}
			//売上ファイルの中身の商品コードがHashmapに格納されているものと異なる場合エラー表示
			if(!commodityName.containsKey(rcdFile.get(commodityIndex))){
	...
	//売上金額合計のメソッド//
	private static boolean fileCalcurate(HashMap<String, Long> valueName, ArrayList<String> fileName, int codeIndex){
		//元々入ってる金額
		Long amount = valueName.get(fileName.get(codeIndex));
		//売上//
		Long sales = Long.valueOf(fileName.get(2));
		//元金額に売上を足す
		Long amountSales = amount + sales;
		//上書きしてMapに格納
		valueName.put(fileName.get(codeIndex), amountSales);

		//合計金額の桁数が10桁以上の場合エラーを表示する
		if(valueName.get(fileName.get(codeIndex)) >= 10000000000L){
...
class MyFilter implements FilenameFilter{
	...
	public boolean accept(File dir, String name){
		...
		if(salesFile.isFile() && ext.equals("rcd") == true){
			return true;
		}
